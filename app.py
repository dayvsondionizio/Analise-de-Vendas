...
def parse_sped_xlsx(arquivo) -> pd.DataFrame:
    """Lê planilha de itens exportada do sistema fiscal (SPED/ERP).

    Formato esperado: cabeçalho na linha 1, colunas incluindo
    'Natureza' (CFOP), 'Valor Contábil', 'Nome' (fornecedor),
    'Item' (produto), 'NCM', 'Documento', 'Data Emissão'.

    Normaliza o CFOP para 4 dígitos (ex: '1403001' → '1403').
    Retorna DataFrame com colunas padronizadas compatíveis com
    calc_simples_nacional().
    """
    try:
        raw = arquivo.read() if hasattr(arquivo, "read") else arquivo
        import io as _io_sped
        df_raw = pd.read_excel(_io_sped.BytesIO(raw), sheet_name=0, header=0)
        # Header real está na linha 0
        df_raw.columns = df_raw.iloc[0].tolist()
        df_raw = df_raw.iloc[1:].reset_index(drop=True)

        # Mapeamento flexível de colunas (funciona mesmo com acentuação diferente)
        col_map = {}
        for col in df_raw.columns:
            c = str(col).lower()
            if "natureza" in c:                      col_map["CFOP_raw"] = col
            elif "valor cont" in c:                  col_map["vProd"] = col
            elif "nome" in c and "filial" not in c:  col_map["emitente"] = col
            elif "item" in c and "c" not in c[:3]:   col_map["xProd"] = col
            elif "ncm" in c:                         col_map["NCM"] = col
            elif "documento" in c:                   col_map["nNF"] = col
            elif "emiss" in c:                       col_map["dhEmi"] = col
            elif "cnpj" in c and "filial" not in c and "cliente" not in c: col_map["cnpj_emit"] = col
            elif "c" in c[:2] and "digo item" in c: col_map["cProd"] = col
        ...
        if df.empty or "CFOP_raw" not in df.columns:
            return pd.DataFrame()

        # Normaliza CFOP: remove não-dígitos, pega 4 primeiros
        df["CFOP"] = df["CFOP_raw"].astype(str).apply(
            lambda c: re.sub(r"[^\d]", "", c)[:4]
        )
        df["CFOP_orig"] = df["CFOP_raw"].astype(str).str.strip()

        # Valor contábil
        if "vProd" in df.columns:
            df["vProd"] = pd.to_numeric(df["vProd"], errors="coerce").fillna(0.0)
        ...
        # Fonte
        df["fonte"] = "sped_xlsx"
        df["chave"] = df.get("nNF", pd.Series(dtype=str)).astype(str)

        return df[df["CFOP"].str.len() == 4].reset_index(drop=True)
...
def calc_simples_nacional(df_entradas: pd.DataFrame, faturamento_total: float,
    ...
    Se df_sped (planilha do sistema fiscal) for fornecido, usa como fonte
    primária (tem a classificação correta do analista/contador).
    df_entradas (XMLs) é usado como fonte secundária ou para confronto.
    ...
    """
    tem_xml  = df_entradas is not None and not df_entradas.empty
    tem_sped = df_sped is not None and not df_sped.empty

    if not tem_xml and not tem_sped:
        return {
    ...
    # ── Fonte primária: planilha SPED (tem classificação do analista) ──────
    if tem_sped:
        df_com_sped = df_sped[df_sped["CFOP"].isin(CFOPS_COMERCIALIZACAO)].copy()
        total = df_com_sped["vProd"].sum()
        fonte = "ambos" if tem_xml else "sped_xlsx"

        # Breakdown por CFOP original (preserva sub-código para info)
        _grp_col = "CFOP_orig" if "CFOP_orig" in df_com_sped.columns else "CFOP"
        df_por_cfop = (
            df_com_sped.groupby(_grp_col)
            .agg(
                total_compras=("vProd", "sum"),
                itens=("vProd", "count"),
            )
            .reset_index()
            .rename(columns={_grp_col: "CFOP"})
            .sort_values("total_compras", ascending=False)
        )
        # notas: por nNF se disponível
        if "nNF" in df_com_sped.columns:
            _notas = df_com_sped.groupby(_grp_col)["nNF"].nunique().rename("notas")
            df_por_cfop = df_por_cfop.merge(_notas.reset_index().rename(columns={_grp_col:"CFOP"}),
                                            on="CFOP", how="left")
        else:
            df_por_cfop["notas"] = 0
        ...
        if _forn_col:
            df_por_fornecedor = (
                ...
                .agg(total_compras=("vProd", "sum"), itens=("vProd", "count"))
                .reset_index()
                .rename(columns={_forn_col: "emitente"})
                .sort_values("total_compras", ascending=False)
                .head(20)
            )
        else:
        ...
        # ── Confronto com XMLs (quando ambos disponíveis) ──────────────────
        df_confronto = pd.DataFrame()
        df_por_cfop_xml = pd.DataFrame()
        if tem_xml:
        ...
        df_excluidos = df_sped[df_sped["CFOP"].isin(_excluir_cfops)].copy() if tem_sped else pd.DataFrame()
    ...
    status = "EXCEDIDO" if pct > 80 else ("ALERTA" if pct > 70 else "OK")
    ...
    return {
        ...
        "df_por_cfop_xml": df_por_cfop_xml,
        "df_por_fornecedor": df_por_fornecedor,
        "df_entradas_filtradas": df_com_sped,
                                ...
                                for _entry in _rf.namelist():
                                    try:
                                        resultado.extend(extrair_xml_bytes(
                                            _rf.read(_entry), _entry.split("/")[-1], excluir))
                                    except Exception:
                                        pass
                        except Exception:
                            pass
...
def calc_curva_abc(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Colunas mascaradas por tipo de nota para cálculo separado
    _df2 = _df2.copy()
    _df2["_rec_nfce"]   = _df2["_rec"].where(_df2["fonte"] == "NFC-e", 0.0)
    _df2["_rec_nfe"]    = _df2["_rec"].where(_df2["fonte"] == "NF-e",  0.0)
    _df2["_chave_nfce"] = _df2["chave"].where(_df2["fonte"] == "NFC-e", None)
    _df2["_chave_nfe"]  = _df2["chave"].where(_df2["fonte"] == "NF-e",  None)
    ...
    prod = (
        ...
        .agg(
            frequencia  =("chave",       "nunique"),
            receita     =("_rec",        "sum"),
            freq_nfce   =("_chave_nfce", "nunique"),
            freq_nfe    =("_chave_nfe",  "nunique"),
            rec_nfce    =("_rec_nfce",   "sum"),
            rec_nfe     =("_rec_nfe",    "sum"),
    ...
    prod = prod.rename(columns={
        ...
        "receita":      "Receita (R$)",
        "freq_nfce":    "Freq. NFC-e",
        "freq_nfe":     "Freq. NF-e",
        "rec_nfce":     "Receita NFC-e",
        "rec_nfe":      "Receita NF-e",
        "pct_receita":  "% Receita",
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        def _escreve_abc(df_src, sheet_name):
            ...
            _cols = ["Rank","Produto","Curva",
                     ...
                     "Receita (R$)","Receita NFC-e","Receita NF-e",
                     "% Receita","% Acumulado","Origem"]
            _cols = [c for c in _cols if c in df_src.columns]
        ...
        if sn_result is not None and sn_result.get("status") not in (None, "SEM_DADOS"):
            _sn_fonte_label = {
                ...
                "xml":       "XMLs de entrada (NF-e fornecedores)",
            }.get(sn_result.get("fonte", "xml"), "XML")
            ...
            if not _df_items.empty:
                ...
                _cols_desejadas = {
                    ...
                    "CFOP": "CFOP 4d", "xProd": "Produto", "NCM": "NCM",
                    "qCom": "Qtd", "vUnCom": "Vl Unit", "vProd": "Vl Total",
                }
...
def exportar_pdf_compras(df_compras: pd.DataFrame, cliente: str, periodo: str) -> bytes:
    """Gera relatório PDF de Compras com matplotlib — mesmo estilo do PDF de Vendas."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    ...
    ev_c    = calc_evolucao_mensal_compras(df_compras)
    forn_c  = calc_ranking_fornecedores_compras(df_compras)
    prod_c  = calc_ranking_produtos_compras(df_compras)
    ...
    with PdfPages(buf) as pdf:
        ...
        def new_slide(title_text=None, has_logo=True):
            ...
            ax_ft.set_facecolor(AZUL)
            ax_ft.axis("off")
            ax_ft.text(0.5, 0.5, f"{cliente}  ·  {periodo}  ·  Análise de Compras CP",
                       color="white", fontsize=12, va="center", ha="center",
                       transform=ax_ft.transAxes)
            return fig
        ...
        def draw_table(fig, rect, data, col_headers, col_widths=None, title=None):
            ax = fig.add_axes(rect)
            ax.axis("off")
            if title:
                ax.text(0, 1.04, title, fontsize=13, fontweight="bold",
        ...
        ax_c.set_facecolor(AZUL)
        ax_c.axis("off")
        ax_c.text(0.5, 0.75, "ANÁLISE ESTRATÉGICA DE COMPRAS",
                  color="white", fontsize=24, fontweight="bold",
                  ha="center", va="center", transform=ax_c.transAxes)
        ax_c.text(0.5, 0.48, cliente.upper(),
                  color="#60a5fa", fontsize=20, fontweight="bold",
        ...
        ax_ft2.set_facecolor("#0f1f3a")
        ax_ft2.axis("off")
        ax_ft2.text(0.5, 0.5, "Análise de Vendas CP  ·  Dados Fiscais NF-e Entradas",
                    color="white", fontsize=13, ha="center", va="center",
                    transform=ax_ft2.transAxes, alpha=0.7)
        pdf.savefig(fig, bbox_inches="tight")
        plt.close(fig)
        ...
        # Slide 2 — KPIs Principais
        # ════════════════════════════════════════════════════════════
        fig = new_slide("INDICADORES GERAIS DE COMPRAS")
        kpi_items = [
            (brl(kpis_c["total_compras"]),        "TOTAL EM COMPRAS",    "", AZUL),
            (fmt_num(kpis_c["n_notas"]),           "NOTAS FISCAIS",       "", AZUL2),
            (fmt_num(kpis_c["n_fornecedores"]),    "FORNECEDORES",        "", "#0369a1"),
            (fmt_num(kpis_c["n_produtos"]),        "PRODUTOS ÚNICOS",     "", "#065f46"),
            (brl(kpis_c["ticket_medio_nota"]),     "TICKET MÉDIO/NF",     "", "#7c3aed"),
        ]
        n = len(kpi_items)
        box_w = 0.17
        gap   = (1 - n * box_w) / (n + 1)
        for i, (val, lbl, sub, clr) in enumerate(kpi_items):
            x = gap + i * (box_w + gap)
            add_kpi_box(fig, x, 0.52, box_w, 0.30, lbl, val, sub, clr)

        # Top 5 fornecedores em barra horizontal
        if not forn_c.empty and "Total (R$)" in forn_c.columns:
            _col_forn = "Fornecedor" if "Fornecedor" in forn_c.columns else forn_c.columns[0]
            top5_f = forn_c.nlargest(5, "Total (R$)")
            ax_b = fig.add_axes([0.05, 0.09, 0.52, 0.36])
            ax_b.set_facecolor("white")
            bars = ax_b.barh(
                top5_f[_col_forn].str[:28][::-1],
                top5_f["Total (R$)"][::-1],
                color=AZUL2, edgecolor="none", height=0.6,
            )
            ax_b.set_xlabel("Total Compras (R$)", fontsize=12, color=SUBTXT)
            ax_b.tick_params(labelsize=9)
            ax_b.spines[["top", "right", "left"]].set_visible(False)
            ax_b.set_title("Top 5 Fornecedores", fontsize=12,
                           fontweight="bold", color=AZUL, pad=6)
            for bar in bars:
                w = bar.get_width()
                ax_b.text(w * 1.01, bar.get_y() + bar.get_height() / 2,
                          brl(w), va="center", fontsize=11, color=TEXTO)
        ...
        # ════════════════════════════════════════════════════════════
        # Slide 3 — Evolução Mensal  (só se > 1 mês)
        # ════════════════════════════════════════════════════════════
        if not ev_c.empty and len(ev_c) > 1:
            fig = new_slide("EVOLUÇÃO MENSAL DE COMPRAS")
            # Gráfico de barras
            ax_ev = fig.add_axes([0.05, 0.42, 0.55, 0.44])
            _meses_lbl = ev_c["Mês"].tolist()
            _totais    = ev_c["Total (R$)"].tolist()
            bars = ax_ev.bar(_meses_lbl, _totais, color=AZUL2, edgecolor="none", width=0.6)
            ax_ev.set_ylabel("Total (R$)", fontsize=12, color=SUBTXT)
            ax_ev.tick_params(labelsize=10)
            ax_ev.spines[["top", "right"]].set_visible(False)
            ax_ev.set_title("Total de Compras por Mês", fontsize=13,
                            fontweight="bold", color=AZUL, pad=8)
            for bar in bars:
                h = bar.get_height()
                ax_ev.text(bar.get_x() + bar.get_width() / 2, h * 1.01,
                           brl(h), ha="center", va="bottom", fontsize=10, color=TEXTO)

            # Tabela mensal
            _ev_tbl = ev_c[["Mês", "Total (R$)", "Nº Notas", "Produtos Distintos"]].copy()
            _ev_tbl["Total (R$)"] = _ev_tbl["Total (R$)"].apply(brl)
            _ev_tbl["Nº Notas"]   = _ev_tbl["Nº Notas"].apply(fmt_num)
            tbl_data = _ev_tbl.values.tolist()
            draw_table(fig, [0.63, 0.42, 0.34, 0.44], tbl_data,
                       list(_ev_tbl.columns),
                       col_widths=[0.22, 0.30, 0.20, 0.28],
                       title="Resumo Mensal")

            # KPIs de variação (último mês vs penúltimo)
            if len(ev_c) >= 2:
                _ult = ev_c.iloc[-1]
                _ant = ev_c.iloc[-2]
                _var = (_ult["Total (R$)"] - _ant["Total (R$)"]) / _ant["Total (R$)"] * 100 if _ant["Total (R$)"] else 0
                _sinal = "▲" if _var >= 0 else "▼"
                _cor   = VERDE if _var >= 0 else "#ef4444"
                ax_v = fig.add_axes([0.05, 0.09, 0.55, 0.26])
                ax_v.set_facecolor(CINZA)
                ax_v.axis("off")
                ax_v.text(0.5, 0.65,
                          f"{_sinal} {abs(_var):.1f}% em relação a {_ant['Mês']}",
                          color=_cor, fontsize=16, fontweight="bold",
                          ha="center", va="center", transform=ax_v.transAxes)
                ax_v.text(0.5, 0.30,
                          f"Último mês ({_ult['Mês']}): {brl(_ult['Total (R$)'])}  ·  "
                          f"Anterior ({_ant['Mês']}): {brl(_ant['Total (R$)'])}",
                          color=SUBTXT, fontsize=13, ha="center", va="center",
        ...
        # ════════════════════════════════════════════════════════════
        # Slide 4 — Top Fornecedores
        # ════════════════════════════════════════════════════════════
        if not forn_c.empty:
            fig = new_slide("RANKING DE FORNECEDORES")
            _col_forn = "Fornecedor" if "Fornecedor" in forn_c.columns else forn_c.columns[0]
            _top_f = forn_c.head(20).copy()
            # Colunas a exibir
            _cols_show = [_col_forn, "Total (R$)", "Nº Notas", "% do Total"]
            _cols_show = [c for c in _cols_show if c in _top_f.columns]
            _top_f_show = _top_f[_cols_show].copy()
            if "Total (R$)" in _top_f_show.columns:
                _top_f_show["Total (R$)"] = _top_f_show["Total (R$)"].apply(brl)
            if "% do Total" in _top_f_show.columns:
                _top_f_show["% do Total"] = _top_f_show["% do Total"].apply(lambda v: f"{v:.1f}%")
            if "Nº Notas" in _top_f_show.columns:
                _top_f_show["Nº Notas"] = _top_f_show["Nº Notas"].apply(fmt_num)
            tbl_data = [
                [str(i + 1)] + [str(v)[:45] if isinstance(v, str) and len(str(v)) > 45
                                else str(v) for v in row]
                for i, row in enumerate(_top_f_show.values.tolist())
            ]
            _n_f = len(tbl_data)
            if _n_f <= 10:
                draw_table(fig, [0.03, 0.07, 0.94, 0.80], tbl_data,
                           ["#"] + _cols_show,
                           col_widths=[0.04, 0.52, 0.18, 0.14, 0.12],
                           title=f"Top {_n_f} Fornecedores por Volume de Compras")
            else:
                _half = (_n_f + 1) // 2
                _cw_l = [0.04, 0.46, 0.22, 0.16, 0.12]
                draw_table(fig, [0.02, 0.07, 0.47, 0.80], tbl_data[:_half],
                           ["#"] + _cols_show,
                           col_widths=_cw_l[:len(_cols_show) + 1],
                           title=f"Fornecedores 1–{_half}")
                draw_table(fig, [0.51, 0.07, 0.47, 0.80], tbl_data[_half:],
                           ["#"] + _cols_show,
                           col_widths=_cw_l[:len(_cols_show) + 1],
                           title=f"Fornecedores {_half + 1}–{_n_f}")
            pdf.savefig(fig, bbox_inches="tight")
            plt.close(fig)

        # ════════════════════════════════════════════════════════════
        # Slide 5 — Curva ABC de Produtos
        # ════════════════════════════════════════════════════════════
        if not prod_c.empty:
            fig = new_slide("CURVA ABC — PRODUTOS COMPRADOS")
            _top_p = prod_c.head(20).copy()
            _cols_p = ["Rank", "Produto", "ABC", "Total (R$)", "Qtd. Total", "% do Total"]
            _cols_p = [c for c in _cols_p if c in _top_p.columns]
            _top_p_show = _top_p[_cols_p].copy()
            if "Total (R$)" in _top_p_show.columns:
                _top_p_show["Total (R$)"] = _top_p_show["Total (R$)"].apply(brl)
            if "% do Total" in _top_p_show.columns:
                _top_p_show["% do Total"] = _top_p_show["% do Total"].apply(lambda v: f"{v:.1f}%")
            if "Qtd. Total" in _top_p_show.columns:
                _top_p_show["Qtd. Total"] = _top_p_show["Qtd. Total"].apply(
                    lambda v: f"{v:,.3f}".replace(",", "X").replace(".", ",").replace("X", ".")
                    if isinstance(v, float) else fmt_num(v)
                )
            # Cores de linha por ABC
            _abc_colors = {"A": "#dcfce7", "B": "#fef9c3", "C": "#fee2e2"}
            tbl_data = [[str(v)[:48] if isinstance(v, str) and len(str(v)) > 48 else str(v)
                         for v in row]
                        for _, row in _top_p_show.iterrows()]
            _cw_map = {
                6: [0.05, 0.46, 0.06, 0.18, 0.13, 0.12],
                5: [0.05, 0.52, 0.06, 0.20, 0.17],
                4: [0.06, 0.56, 0.20, 0.18],
            }
            _cw = _cw_map.get(len(_cols_p), [1 / len(_cols_p)] * len(_cols_p))
            _row_clrs = []
            if "ABC" in _top_p.columns:
                for _, r in _top_p.iterrows():
                    _row_clrs.append(_abc_colors.get(str(r.get("ABC", "")), "white"))
            # draw_table com row_colors customizados inline
            ax = fig.add_axes([0.03, 0.07, 0.94, 0.80])
            ax.axis("off")
            ax.text(0, 1.04, "Top 20 Produtos por Volume de Compras (Curva ABC)",
            ...
            for (r, c), cell in tbl.get_celld().items():
                if r == 0:
                    ...
                    bg = _row_clrs[r - 1] if _row_clrs and r - 1 < len(_row_clrs) else (
                        "#eef2ff" if r % 2 == 0 else "white"
                    )
                    cell.set_facecolor(bg)
                    cell.set_edgecolor("#e5e7eb")

            # Legenda ABC
            ax_leg = fig.add_axes([0.75, 0.05, 0.22, 0.07])
            ax_leg.axis("off")
            for _xi, (_lbl, _clr) in enumerate([
                ("A — 80% do valor", "#dcfce7"),
                ("B — 15% do valor", "#fef9c3"),
                ("C — 5% do valor",  "#fee2e2"),
            ]):
                rect = plt.Rectangle((0.01 + _xi * 0.33, 0.1), 0.30, 0.80,
                                     facecolor=_clr, edgecolor="#9ca3af", lw=0.8,
                                     transform=ax_leg.transAxes, clip_on=False)
                ax_leg.add_patch(rect)
                ax_leg.text(0.165 + _xi * 0.33, 0.50, _lbl,
                            fontsize=9, ha="center", va="center",
                            transform=ax_leg.transAxes, color=TEXTO)
...
""",
        ...
        if _tem_entradas:
            ...
            # ── Planilha SPED (fonte primária) ──────────────────────
            if arquivo_sped is not None:
                try:
                    arquivo_sped.seek(0)
                except Exception:
                    pass
                try:
                    df_sped_parsed = parse_sped_xlsx(arquivo_sped)
            ...
            sn_result = calc_simples_nacional(
                ...
                df_sped=df_sped_parsed if not df_sped_parsed.empty else None
            )
...
"</tr></table></div>"
    ...
    if _modo == "Compras" and not df_compras.empty:
        ...
        _col_xls_c, _col_pdf_c = st.columns(2)
        with _col_xls_c:
            st.download_button(
        ...
        with _col_pdf_c:
            _pdf_compras = exportar_pdf_compras(df_compras, cli_label, per_label)
            if _pdf_compras:
                st.download_button(
                    label="⬇️ Baixar PDF de Compras (.pdf)",
                    data=_pdf_compras,
                    file_name=f"{_nome_base_c}.pdf",
                    mime="application/pdf",
                    use_container_width=True,
                )
            else:
                st.info("PDF não disponível (matplotlib ausente).")
...
""")
            ...
            if not df_excluidos.empty:
                st.divider()
                with st.expander(f"🚫 Itens reclassificados pelo analista — **uso e consumo / outros** ({len(df_excluidos)} itens, {brl(df_excluidos['vProd'].sum())} excluídos do cálculo)", expanded=False):
                    st.caption("Estes itens constam na planilha SPED mas foram classificados pelo contador como uso e consumo, ativo imobilizado ou outra natureza — portanto NÃO entram nas compras de comercialização.")
                    _exc_show = df_excluidos.copy()
                    _fmt_cols_exc = {"vProd": brl}
                    for _c, _fn in _fmt_cols_exc.items():
                        if _c in _exc_show.columns:
                            _exc_show[_c] = _exc_show[_c].apply(_fn)
                    _ren_exc = {"CFOP_orig": "Natureza (CFOP)", "CFOP": "CFOP 4d", "vProd": "Valor",
                                "emitente": "Fornecedor", "xProd": "Descrição", "NCM": "NCM",
                                "nNF": "Documento", "dhEmi": "Data Emissão"}
                    _exc_show = _exc_show.rename(columns={k: v for k, v in _ren_exc.items() if k in _exc_show.columns})
                    st.dataframe(_exc_show, use_container_width=True, hide_index=True, height=280)
