...
def processar_fontes_universal(arquivos: tuple, pastas: tuple):
    ...
    # Processa ZIPs em streaming — nunca carrega todos os XMLs na RAM de uma vez
    # Para cada ZIP: itera entradas em lotes de 200, liberando memória entre lotes
    ...
    for nome, data in arquivos:
        ...
        if ext_f == "zip":
            _processar_zip_stream(data, nome)
        else:
            # RAR, 7z, XML individual — extrai tudo e processa em lotes
            try:
...
def exportar_excel_compras(df_compras: pd.DataFrame, cliente: str, periodo: str,
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        _resumo_rows = [
            ...
            ("  → Comercialização",        _kpis_c["n_notas"],                  _FMT_NUM),
            ("  → Outras Entradas",        _kpis_todos["n_notas"] - _kpis_c["n_notas"], _FMT_NUM),
            ("Fornecedores (total)",       _kpis_todos["n_fornecedores"],       _FMT_NUM),
            ("  → Comercialização",        _kpis_c["n_fornecedores"],           _FMT_NUM),
            ("  → Outras Entradas",        _kpis_todos["n_fornecedores"] - _kpis_c["n_fornecedores"], _FMT_NUM),
            ("Produtos Únicos (total)",    _kpis_todos["n_produtos"],           _FMT_NUM),
            ("  → Comercialização",        _kpis_c["n_produtos"],               _FMT_NUM),
            ("  → Outras Entradas",        _kpis_todos["n_produtos"] - _kpis_c["n_produtos"], _FMT_NUM),
            ("Ticket Médio/NF (comerc.)",  _kpis_c["ticket_medio_nota"],        _FMT_BRL),
            ("Obs.",
    ...
    if df_nfe_outros is not None and not df_nfe_outros.empty and "chave" in df_nfe_outros.columns:
        ...
        if "CFOP" in df_nfe_outros.columns:
            ...
            for ri, (_, row) in enumerate(_grp_ot.iterrows()):
                ...
                if _tipo_col and _tem_nat:
                    vals_ot = [str(row.get("_tipo_op", ""))[:28], str(row.get("CFOP", "")),
                               str(row.get("xNatOp", ""))[:48], nn, vl]
                elif _tipo_col:
                    vals_ot = [str(row.get("_tipo_op", ""))[:32], str(row.get("CFOP", "")), nn, vl]
                else:
...
"</tr></table></div>"
    ...
    if _modo == "Compras" and not df_compras.empty:
        ...
        _n_notas_total = calc_kpis_compras(_df_c_todos)["n_notas"]
        _n_outras_nf   = _n_notas_total - _kpis_c["n_notas"]
        _cc2.metric(
            "Notas Fiscais",
            fmt_num(_kpis_c["n_notas"]),
            delta=f"+{fmt_num(_n_outras_nf)} outras entradas = {fmt_num(_n_notas_total)} total" if _n_outras_nf > 0 else None,
            delta_color="off",
            help=(
                ...
                + (f" + {fmt_num(_n_outras_nf)} NFs de outras entradas (equipamentos, uso/consumo etc.)"
                   f" = {fmt_num(_n_notas_total)} total"
